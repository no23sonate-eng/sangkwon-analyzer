...
export default function ComparePage() {
  ...
  return (
    <div className="relative h-full w-full overflow-hidden">
      {/* ── 아래 UI는 비활성화 + 블러. 그 위에 To be continued 오버레이 ── */}
      <div
        aria-hidden="true"
        className="pointer-events-none flex h-full select-none"
      ...
      {/* ── To be continued 오버레이 — 전체 페이지 차단 ── */}
      <div className="absolute inset-0 z-50 flex items-center justify-center bg-white/30 backdrop-blur-sm">
        <div className="mx-4 max-w-sm rounded-3xl border border-gray-200 bg-white px-8 py-8 text-center shadow-2xl">
          <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-primary-600">
            To be continued
          </p>
          <h2 className="mt-3 text-[18px] font-bold text-gray-900">입지 비교 기능 준비 중</h2>
          <p className="mt-2 text-[13px] leading-relaxed text-gray-500">
            더 정확한 비교 지표를 담아 곧 공개할 예정입니다.
            <br />지금은 지도 분석 기능을 이용해 주세요.
          </p>
          <button
            onClick={() => { window.location.href = "/map"; }}
            className="mt-5 rounded-xl bg-primary-600 px-5 py-2.5 text-[13px] font-semibold text-white transition-colors hover:bg-primary-700"
          >
            지도 분석으로 이동
          </button>
        </div>
