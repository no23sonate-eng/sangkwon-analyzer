...
import {
  computeVolumeStudy, rankSingleUses, ZONE_LIST, LEGAL_REFS, VOLUME_DISCLAIMER,
...
export async function POST(request: Request) {
  ...
  if (mixSum <= 0) {
    return NextResponse.json({ error: "용도 믹스를 1개 이상 지정하세요." }, { status: 400 });
  }

  const options = body.options ?? {};
  const study = computeVolumeStudy(siteAreaM2, zoneKey, mix, options);
