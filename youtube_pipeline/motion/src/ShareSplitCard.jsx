...
export const ShareSplitCard = ({
  ...
  // 판을 왼쪽 끝(150)에 박아 놔서 덩어리가 한쪽으로 쏠렸다 (검수 지적 #61·#71).
  // 판 + 오른쪽 설명을 한 덩어리로 보고 그 가운데를 화면 가운데에 맞춘다.
  const LABW = 560;
  const PX = Math.round((1920 - (PW + 56 + LABW)) / 2);
  ...
  return (
    <AbsoluteFill style={{fontFamily: 'A2Z Regular, sans-serif'}}>
      ...
      <div style={{position: 'absolute', left: PX, top: PY - 62, opacity: fadeIn(frame, 8),
                   fontFamily: 'A2Z Medium, sans-serif', fontSize: 36,
                   color: T.ink}}>
        {ownerLabel} {n}명 · 한 필지
      </div>
