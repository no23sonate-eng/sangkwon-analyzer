    ...
    <AbsoluteFill style={{fontFamily: 'A2Z Regular, sans-serif'}}>
      ...
      {[['above', above, groundY - H_ABOVE / 2], ['below', below, groundY + 10]].map(([k, o, y]) => (
        ...
        <div key={k} style={{position: 'absolute', left: DIM_X - 28 - 420, top: y, width: 420,
          ...
          <div style={{fontFamily: 'A2Z Light, sans-serif', fontSize: 31, color: T.soft, letterSpacing: '0.05em', lineHeight: 1.1}}>{o.label}</div>
          <div style={{fontFamily: 'A2Z Medium, sans-serif', fontSize: 56, color: T.ink, lineHeight: 1.08}}>
            {o.floors}<span style={{fontSize: 38}}>층</span>
          </div>
          {o.note ? (
            <div style={{marginTop: 4, fontFamily: 'A2Z Light, sans-serif', fontSize: 31, color: T.soft, lineHeight: 1.2}}>{o.note}</div>
          ) : null}
        </div>
      ))}
