    ...
    <AbsoluteFill style={{fontFamily: 'A2Z Regular, sans-serif'}}>
      ...
      <svg width={1920} height={1080} style={{position: 'absolute', top: 0, left: 0}}>
        ...
        <path d={`M ${mx - mtW / 2} ${groundY} Q ${mx - mtW * 0.22} ${mTop + 30} ${mx} ${mTop}
              ...
              fill={TONES[2]} opacity={fadeIn(frame, 6)} />
        {/* 7부능선 표시 */}
        <line x1={mx - mtW * 0.30} y1={ridgeY} x2={mx + mtW * 0.34} y2={ridgeY}
              stroke={INK} strokeWidth={2} strokeDasharray="6 6" opacity={0.55 * fadeIn(frame, 18)} />
        ...
        <g opacity={fadeIn(frame, 12)}>
          ...
          <rect x={building.x - building.w / 2} y={groundY - building.h}
                width={building.w} height={building.h} fill="none" stroke={INK} strokeWidth={3} />
        ...
        <line x1={viewer.x} y1={eyeY} x2={lineX} y2={lineY}
              stroke={INK} strokeWidth={2.6} strokeDasharray="9 7" />
        {drawT > 0.98 ? (
          <circle cx={mx} cy={ridgeY} r={7} fill={INK} opacity={fadeIn(frame, 76)} />
        ) : null}

        {/* 관측자 */}
        <g opacity={fadeIn(frame, 2)}>
          <circle cx={viewer.x} cy={eyeY - 26} r={13} fill={INK} />
          <path d={`M ${viewer.x - 20} ${groundY} L ${viewer.x - 20} ${eyeY - 12}
                    A 20 20 0 0 1 ${viewer.x + 20} ${eyeY - 12} L ${viewer.x + 20} ${groundY} Z`} fill={INK} />
        </g>

        {/* 지반 */}
        <line x1={90} y1={groundY} x2={1830} y2={groundY} stroke={INK} strokeWidth={4} />
      </svg>
      ...
      <div style={{position: 'absolute', left: viewer.x - 210, width: 420, top: groundY + 22, textAlign: 'center',
                   ...
                   fontSize: 32, color: INK, wordBreak: 'keep-all'}}>
        {viewer.label}
      </div>
      ...
      <div style={{position: 'absolute', left: mx - 240, width: 480, top: groundY - mtH - 112, textAlign: 'center',
                   opacity: fadeIn(frame, 66), fontFamily: 'A2Z Regular, sans-serif', fontSize: 30, color: INK}}>
        <span style={{background: 'rgba(250,255,46,0.85)', padding: '4px 14px'}}>{peak.ridgeLabel}</span>
        <div style={{marginTop: 10, fontFamily: 'A2Z Light, sans-serif', fontSize: 26, color: INK_SOFT, whiteSpace: 'nowrap'}}>
          이 선이 보여야 한다
        </div>
      </div>
      {building.label ? (
        <div style={{position: 'absolute', left: building.x - 230, width: 460, top: groundY - building.h - 78,
                     textAlign: 'center', opacity: fadeIn(frame, 20)}}>
          <div style={{fontFamily: 'Pretendard Bold, A2Z Medium, sans-serif', fontSize: 34, color: INK}}>{building.label}</div>
          {building.note ? (
            <div style={{marginTop: 6, fontFamily: 'A2Z Light, sans-serif', fontSize: 26, color: INK_SOFT}}>{building.note}</div>
          ) : null}
        </div>
      ) : null}
