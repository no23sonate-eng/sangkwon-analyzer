...
import {PaperBg, PaperTitle, PaperSource, themeOf, INK, YELLOW, fadeIn, LW, stageTop, titleH} from './paper';

// 좌우 대비 카드 — 두 항목을 한 화면에서 갈라 놓고 비교한다.
// 아이콘 3개를 좌→우로 세우는 것과 달리, "둘 사이의 간극"이 주어다.
// left/right: {label, sub, lines:[...], hot}
const Col = ({d, x, w, frame, delay, T, colTop}) => {
  const o = fadeIn(frame, delay);
  return (
    <div style={{position: 'absolute', left: x, width: w, top: colTop, opacity: o, textAlign: 'center'}}>
      ...
      {d.sub ? (
        <div style={{marginTop: 14, fontFamily: 'A2Z Light, sans-serif', fontSize: 36, color: T.soft}}>
          {d.sub}
        </div>
      ) : null}
...
export const SplitCard = ({title = '', sub = '', left = {}, right = {}, verdict = '',
  ...
  // 좌우 칸이 320 에 못 박혀 위에 붙어 있었다. 제목 아래에서
  // 화면 가운데로 내린다 (칸 높이는 라벨+보조설명 두 줄 기준)
  const colTop = stageTop(titleH(title, sub) + 64 + 190, {top: 200})
    + titleH(title, sub) + 64;
  ...
  return (
    <AbsoluteFill style={{fontFamily: 'A2Z Regular, sans-serif'}}>
      ...
      <Col colTop={colTop} d={right} x={HALF + PAD * 0.5} w={HALF - PAD * 1.5} frame={frame} delay={20} T={T} />
      {verdict ? (
