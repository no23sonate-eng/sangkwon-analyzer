...
export const AreaNestCard = ({
  ...
  if (!items.length) return <AbsoluteFill><PaperBg /></AbsoluteFill>;
          ...
          return (
            <g key={i}>
              <rect x={x} y={BASE_Y - s} width={s} height={s}
                    fill={it.hot ? YELLOW : TONES[(i * 2 + 1) % TONES.length]}
                    opacity={it.hot ? 0.92 : 0.55} />
              <rect x={x} y={BASE_Y - s} width={s} height={s}
                    fill="none" stroke={INK} strokeWidth={i === 0 ? 3 : 2.5}
          ...
          <React.Fragment key={i}>
            <svg width={1920} height={1080} style={{position: 'absolute', top: 0, left: 0, opacity: o}}>
              <line x1={xRight} y1={yTop} x2={CX + BOX / 2 + 84} y2={yTop}
                    stroke={INK} strokeWidth={2} opacity={0.45} strokeDasharray="5 5" />
              <circle cx={xRight} cy={yTop} r={5} fill={INK} opacity={0.6} />
            </svg>
            <div style={{position: 'absolute', left: CX + BOX / 2 + 100, width: 700, top: yTop,
                         transform: 'translateY(-50%)', opacity: o}}>
              <div style={{fontFamily: 'Pretendard Bold, A2Z Medium, sans-serif', fontSize: 62, color: INK,
