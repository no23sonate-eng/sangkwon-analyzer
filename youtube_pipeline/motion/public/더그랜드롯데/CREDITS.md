# 자료 출처

| 파일 | 원본 | 출처 / 라이선스 | 화면 표기 |
|---|---|---|---|
| `clinic_modern.mp4` | Pexels video 3197634 | Pexels License | (표기 의무 없음) |
| `_t.jpg` | Pexels photo 18426842 | Pexels License | (표기 의무 없음) |
| `hotel_city_block.jpg` | Pexels photo 34597463 | Pexels License | (표기 의무 없음) |
| `seoulstation_north_cgi.jpg` | ㈜한화 건설부문 뉴스룸 2025.9.4 | **저작권물 · 출처 표기 사용** | `㈜한화 건설부문` |
| `mo_ny_lounge.jpg` | ㈜한화 건설부문 뉴스룸 2025.9.4 | **저작권물 · 출처 표기 사용** | `㈜한화 건설부문` |
| `seoulstation_north_aerial.jpg` | 파이낸셜뉴스 2025.9.4 (한화 건설부문 제공) | **저작권물 · 출처 표기 사용** | `한화 건설부문 · 파이낸셜뉴스` |
| `mo_tokyo_lobby.jpg` | 한국일보 2025.9.5 (한화 건설부문 제공) | **저작권물 · 출처 표기 사용** | `한화 건설부문 · 한국일보` |
| `seoulstation_north_hk.jpg` | 한국일보 2025.9.5 (한화 건설부문 제공) | **저작권물 · 출처 표기 사용** | `한화 건설부문 · 한국일보` |
| `hotel_suite_view.jpg` | Pexels photo 271624 | Pexels License | (표기 의무 없음) |
| `interior_luxe_build.mp4` | Pexels video 31617692 | Pexels License | (표기 의무 없음) |
| `blueprint_closeup.mp4` | Pexels video 8471104 | Pexels License | (표기 의무 없음) |
| `logo_rosewood.svg` | Rosewood Hotels & Resorts 공식 워드마크 | **상표 · 출처 표기 사용** | `로즈우드` |
| `logo_aman.svg` | Aman 공식 워드마크 | **상표 · 출처 표기 사용** | `아만` |
