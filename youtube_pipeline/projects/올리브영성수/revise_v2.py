...
OLIVE_N = (37.544207, 127.054377)      # 연무장7길 13 · 팩토리얼 성수
# ↑ 예전 값(37.543793, 127.054720)은 주소 지오코딩과 52m 어긋나 있었다.
# #11 지도와 #77 지도가 같은 매장을 다른 자리에 찍고 있었다는 뜻이라 통일한다.
# ── 올리브영 성수 상권 6곳 ──────────────────────────────────────────────
# **한 곳에서만 정의한다.** 예전엔 #11 과 #77 이 같은 목록을 따로 들고 있었고,
# 한쪽만 고치는 바람에 같은 매장이 두 지도에서 다른 자리에 찍혀 있었다.
# 좌표는 매장별 도로명 주소를 지오코딩한 값 (주소=네이버 검색, 좌표=OSM).
OY_STORES = [
    {'lat': 37.545258, 'lon': 127.054493, 'label': '성수역점'},      # 아차산로 91 현진빌딩
    {'lat': 37.547694, 'lon': 127.045124, 'label': '뚝섬역점'},      # 아차산로 6
    {'lat': OLIVE_N[0], 'lon': OLIVE_N[1], 'label': '올리브영N 성수', 'hot': True},
    {'lat': 37.544551, 'lon': 127.057387, 'label': '뷰티 맨션 성수'},  # 성수이로 108 2층
    {'lat': 37.541900, 'lon': 127.044976, 'label': '서울숲역점'},     # 왕십리로 50
    {'lat': 37.541405, 'lon': 127.056912, 'label': '성수연방점'},     # 성수이로14길 14
]
...
# #120 은 갈림길로 되돌린다.
# "같은 포맷이 또 있다" 는 지적을 받고 좌우 판(TwoPanelCard)으로 바꿨는데,
# 정작 #97 도 좌우 판으로 만들어 놓는 바람에 **똑같은 그림이 두 번** 나갔다.
# 고치려다 같은 자리로 돌아온 셈이다. 두 컷은 뜻부터 다르다 —
#   #97 : 임대 수익 **과** 브랜드 효과. 둘 다 살아 있다  → 좌우 판이 맞다
#   #120: 따라갈지 **아니면** 만들지. 하나를 고른다     → 갈림길이 맞다
# 그림이 문장을 따라가야지, 그 반대가 되면 안 된다.
R[120] = ('ForkPathCard', {
    'title': '앞으로의 두 갈래', 'sub': '',
    'trunkLabel': '올리브영 성수 부지',
    'branches': [{'label': '따라간다', 'note': '다른 브랜드가 이미 보여준 방식'},
                 {'label': '만든다', 'note': '올리브영만의 공간 콘텐츠'}],
    'question': '어느 쪽?',
    # #97 이 blueprint 라 여기는 먹으로 — 배경까지 같으면 또 같은 컷으로 읽힌다
    'theme': 'ink', 'source': '도해'})
...
# ── 흰 베일이 모자란 컷 (측정으로 잡았다) ────────────────────────────────
# tone='light' 는 **흰 베일 위에 검정 글자**다. 베일이 얇으면 사진이 그대로
# 비쳐 글자가 회색 얼룩에 묻힌다. 눈으로는 "좀 흐린가?" 정도로 넘어가는데
# 실제로 재 보니 글자 뒤 밝기가 147~173 이었다 — 검정 글자가 편히 읽히려면
# 195 는 넘어야 한다. scripts/check_contrast.py 가 이 값을 뽑는다.
for _sid, _sc in ((17, 0.67), (21, 0.74), (29, 0.76), (37, 0.78),
                  (83, 0.62), (91, 0.78), (107, 0.75), (115, 0.65)):
    if _sid in R and isinstance(R[_sid], tuple):
        R[_sid][1]['scrim'] = _sc

# ── 옆 컷의 자막을 물고 온 컷 ────────────────────────────────────────────
# 소재를 갈아 끼우면서 라벨까지 통째로 복사한 자리가 있다. 자막이 자기 나레이션이
# 아니라 **옆 컷의 문장**을 달고 나갔다. 화면만 보면 안 보이고, 나레이션과
# 나란히 놓아야 드러난다 (#109 는 #110 의 문장을, #90 은 #106 의 문장을 달고 있었다).
_FIX = {
    # 60/61 은 같은 말을 두 컷 연속으로 띄우고 있었다 — 뒤 컷은 그림의 이름으로 바꾼다
    61: ('title', '한 건물, 열아홉 주인'),
    # "그렇다면 올리브영이 내는 임대료도 향후 올라갈 가능성이 있습니다"
    90: ('label', '올리브영도 예외가 아니다'),
    # "제품과 트렌드를 직접 경험하게 만드는 공간."
    109: ('label', '직접 경험하게 만드는 공간'),
}
def _apply_fix(props):
    """R 에 없는 컷(기본 생성분)도 고쳐야 해서 최종 props 를 직접 손본다."""
    for _sid, (_k, _v) in _FIX.items():
        _e = props['scenes'].get(str(_sid))
        if _e:
            _e['props'][_k] = _v

# ── 전환 모션은 18컷에만 ─────────────────────────────────────────────────
# 예전엔 123컷 전부가 밀려 들어왔다 (MotionWrap 의 기본값 때문이라 motion 을
# 안 준 컷도 움직였다). **전환이 늘 있으면 전환이 아무 뜻도 없다** —
# 컷이 바뀐다는 사실 자체는 이미 화면이 바뀌는 걸로 충분히 전달된다.
# 움직임은 "여기서부터 다른 이야기" 라고 말할 때만 쓴다.
#
# 남기는 자리는 두 종류뿐이다.
#   ① 챕터가 바뀌는 첫 컷 (8곳) — 장이 넘어간다는 신호
#   ② 판이 뒤집히는 자리 (10곳) — 낙찰가 · 낙찰가율 · 최고가 · 결론
# 방향도 뜻을 따라간다: 새 장은 왼쪽에서 밀고 들어오고(전진),
# 숫자가 커지는 컷은 아래에서 올라온다(상승).
_MOTION = {
    # ① 챕터 시작
    0:   ('left', 0.045), 15:  ('left', 0.04), 32:  ('left', 0.04),
    47:  ('left', 0.04), 64:  ('left', 0.04), 79:  ('left', 0.04),
    92:  ('left', 0.04), 108: ('left', 0.04),
    # ② 판이 뒤집히는 자리
    2:   ('up', 0.05),    # 541.5억 — 이 영상의 첫 충격
    4:   ('up', 0.045),   # 감정가 대비 +145억 막대
    8:   ('up', 0.045),   # 136.77%
    18:  ('right', 0.04), # 건물이 네모반듯하다 — 설명의 시작
    39:  ('down', 0.04),  # 층고가 낮으면 (줄어드는 모션과 방향을 맞춘다)
    69:  ('up', 0.045),   # 2,202억 — 경매 사상 최고가
    84:  ('up', 0.045),   # 2,548억 — 성수 오피스 최고가
    98:  ('up', 0.045),   # 오피스로 지으면 7~8층
    117: ('left', 0.04),  # 브랜드마다 전략이 다르다 — 정리 표
    120: ('right', 0.05), # 앞으로의 두 갈래 — 마지막 질문
}


def _apply_motion(props):
    for _k, _e in props['scenes'].items():
