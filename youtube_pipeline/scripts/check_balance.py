...
SOURCE_STRIP = 100
FRAME = 40             # 모서리 재단선이 사는 가장자리 띠 — 내용이 아니다
GRAD = 90
...
def main():
    ...
    for f in stills:
        ...
        # 모서리 재단선(CropMarks, 가장자리 18px 안쪽 + 십자 11px)은 판의 서명이지
        # 내용이 아니다. 세면 잉크가 y=1063 까지 닿아 모든 그래픽 컷이 '아래로
        # 쏠림'으로 걸린다 — 실제로 #260·#266 이 그렇게 걸렸다. 가장자리 띠는 뺀다
        e[:FRAME, :] = 0; e[-FRAME:, :] = 0; e[:, :FRAME] = 0; e[:, -FRAME:] = 0
