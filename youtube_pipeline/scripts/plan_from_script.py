...
def add_motion(plan):
    ...
    2026-08-29 — 105컷에 같은 밀기가 걸려 있었다. 방향을 2~3컷마다 바꿔
    흐름을 만든다는 설계였는데 **이어 붙여 보면 그게 어지럽다.** 컷마다
    화면이 통째로 움직이니 눈이 매번 다시 자리를 잡아야 한다.

    없애는 건 화면 전환이지 움직임 전체가 아니다 — 숫자 카운팅, 글자
    떠오름, 막대 자람, 사진의 느린 줌은 카드가 저마다 갖고 있고 그대로다.

    전환이 **신호**가 되는 자리(챕터가 바뀌는 컷 등)에만 손으로
    `motion: {transition: true}` 를 넣는다. 자동으로 깔지 않는다.
    """
    return plan
