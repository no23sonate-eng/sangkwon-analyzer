...
NOTE = {
 ...
 3:('원문','737 → 590 · 점 하나 = 객실 하나 · 비즈워치 2026.8.14'),
 4:('계산','147 ÷ 737 = 19.95%'),
 ...
 88:('원문','DART: 롯데호텔 16 · 롯데시티 8 · L7 8 · 리조트 4 · 시그니엘 2 · 스카이힐CC 2 · 브리브 1 = 41곳. 공시 비중은 이 개수의 비율이다'),
 ...
 109:('도해','실제 월별 통계가 아니다 — 객실 매출과 임대 수입의 차이를 보이기 위한 가정. 화면에 명시'),
 ...
 148:('원문','전 세계 44개 호텔 · 12개 레지던스 · 부채 마크는 본사 사이트'),
 149:('원문','한국 첫 진출'),
...
FIX = [
 ...
 ('없는 값을 만들어 냈다', '#88',
  '"브랜드별 매출 비중" 으로 세웠는데 **매출이 아니라 시설 수**다. 공시의 39% · 19.5% 는 바로 앞 문장의 개수(롯데호텔 16 · L7 8 · … = 41곳)에서 나온 비율이다. 게다가 합이 95% 였다.',
  '개수를 그대로 세웠다 (16 · 8 · 8 · 4 · 2 · 2 · 1). 빠져 있던 스카이힐CC 도 넣었다. 대본 #87 의 "브랜드별 매출 비중" 문장도 "운영 시설을 브랜드별로 세어 보면" 으로 고쳤다 — 자막이 화면과 어긋나면 안 된다.'),
 ('숫자가 안 맞았다', '#83 · #85',
 ...
 ('보이지 않았다', '#3',
  '이름·숫자를 점판 위에 얹어 놓아 정작 센 것(737개 점)이 글자에 가려졌다. 게다가 자리를 가장 큰 그룹(590) 기준으로 잡고 737개를 그려서 아래 두 줄이 자막 영역까지 흘러내렸다.',
  '점판은 왼쪽, 남은 객실 590실 → 사라진 객실 147실을 오른쪽에 세로로 쌓았다. 자리 계산은 이어 붙인 전체 개수로 바꿨다.'),
 ('보이지 않았다', '#162',
  "'이오타 서울' 이 부제와 부모 상자에 두 번 찍히고, 설계사 칸은 '로고 자리' 로 비어 있었다.",
  '포스터앤파트너스 · SOM 워드마크를 각 사 사이트에서 받아 채웠다.'),
 ('보이지 않았다', '#167 · #148 · #159',
  '로고를 흰 칩에 넣어 사진 위에 붙인 스티커처럼 보였고, 종이 무대(#148·#159)에서는 그 흰 사각형이 크림 바탕에 그대로 드러났다. 만다린 파일에는 **아시아뉴스통신 워터마크**까지 박혀 있었고 로고가 위 글자를 덮었다.',
  '네 마크를 모두 누끼 따서 — 사진 위에는 백색, 종이 위에는 먹·브랜드 골드로 — 바탕에 바로 얹었다. 만다린은 본사 사이트의 부채 마크로 교체.'),
 ('보이지 않았다', 'CostStack 전반',
  "표시값이 단위를 두 번 찍었다 — '4,135억' + '억원' = '4,135억억원'.",
  '숫자만 넣고 단위는 카드가 붙인다. 숫자가 아닌 표시는 단위를 뗀다.'),
 ('어지러웠다', '105컷',
  '컷마다 화면 전체가 밀려 들어왔다. 방향을 2~3컷마다 바꿔 흐름을 만든다는 설계였는데, 이어 붙여 보면 눈이 매번 자리를 다시 잡아야 한다. 전환이 늘 있으면 전환이 아무 뜻도 없다.',
  '화면을 미는 동작만 뺐다. 숫자 카운팅·페이드인·막대가 자라는 건 그 컷의 내용이라 그대로 둔다. 덤으로 모든 컷이 1.2% 확대돼 나가던 것도 잡혔다 — 자막 안전선 여백이 그만큼 잘리고 있었다.'),
 ('되살릴 수 없었다', '#2 · #58 · #97',
