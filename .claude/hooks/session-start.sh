...
#
# **ffmpeg 부터다.** 이건 저장소가 아니라 시스템에 깔리는 물건이라
# 컨테이너가 새로 뜨면 통째로 없다. 없으면 소재 영상 내려받기도(변환 단계)
# 클립 렌더도 다 멈추는데, 처음 이 훅을 짤 때 그걸 안 봤다 —
# 실제로 다음 세션에서 `FileNotFoundError: ffmpeg` 로 34개가 다 실패했다.
# 파이썬·노드보다 먼저 본다
if ! command -v ffmpeg >/dev/null 2>&1; then
  say 'ffmpeg 설치'
  (apt-get update -qq && apt-get install -y -qq ffmpeg) >/dev/null 2>&1 \
    || say '  ffmpeg 설치 실패 — 렌더와 소재 내려받기가 안 된다'
fi
